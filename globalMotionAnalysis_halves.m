...
% correlations between recording halves for area-to-motion comparisons.
%
% The following data files are produced:
% dataDir\area2motion\PRsFolder\phaseCorrelationsSubfolder\globalHalfPops_area2motion.mat
...
%   in dataDir\area2motion\PRsFolder\phaseCorrelationsSubfolder.
...
if strcmp(repository,'all')
  if strcmp(subpop, 'all')
    rootFolder = [dataDir filesep area2motionDir filesep PRsFolder];
  elseif strcmp(subpop, 'positive')
    rootFolder = [dataDir filesep area2motionDir_positive filesep PRsFolder];
  elseif strcmp(subpop, 'negative')
    rootFolder = [dataDir filesep area2motionDir_negative filesep PRsFolder];
  end
  animals = animalsOI;
elseif strcmp(repository,'uol')
  if strcmp(subpop, 'all')
    rootFolder = [dataDir filesep area2motionDir_uol filesep PRsFolder];
  elseif strcmp(subpop, 'positive')
    rootFolder = [dataDir filesep area2motionDir_uol_positive filesep PRsFolder];
  elseif strcmp(subpop, 'negative')
    rootFolder = [dataDir filesep area2motionDir_uol_negative filesep PRsFolder];
  end
  animals = animalsUOLOI;
elseif strcmp(repository,'allensdk')
  if strcmp(subpop, 'all')
    rootFolder = [dataDir filesep area2motionDir_allensdk filesep PRsFolder];
  elseif strcmp(subpop, 'positive')
    rootFolder = [dataDir filesep area2motionDir_allensdk_positive filesep PRsFolder];
  elseif strcmp(subpop, 'negative')
    rootFolder = [dataDir filesep area2motionDir_allensdk_negative filesep PRsFolder];
  end
    ...
    for dbCount = 1:numel(fnsData) % Loop through database entries
      ...
      for iAreaPlusAll = area % Loop through the main and pooled areas
        ...
        for iCondPlusAll = condLoop % Loop through the main and pooled conditions
          ...
          coh = dbStruct.popData.motion.popData.coh_halves(1,:);
          coh_conf = dbStruct.popData.motion.popData.coh_conf_halves(1,:);
          rateadjust_kappa = dbStruct.popData.motion.popData.rateadjust_kappa_halves(1,:);
          phase = dbStruct.popData.motion.popData.phase_halves(1,:);
          phase_confU = dbStruct.popData.motion.popData.phase_conf_halves(1,:);
          phase_confL = dbStruct.popData.motion.popData.phase_conf_halves(2,:);
          [phase1, phaseConfU1, phaseConfL1, coh1, cohConfU1, cohConfL1] = correctPhaseCoh(phase,...
          ...
          coh = dbStruct.popData.motion.popData.coh_halves(2,:);
          coh_conf = dbStruct.popData.motion.popData.coh_conf_halves(2,:);
          rateadjust_kappa = dbStruct.popData.motion.popData.rateadjust_kappa_halves(2,:);
          phase = dbStruct.popData.motion.popData.phase_halves(2,:);
          phase_confU = dbStruct.popData.motion.popData.phase_conf_halves(3,:);
          phase_confL = dbStruct.popData.motion.popData.phase_conf_halves(4,:);
          [phase2, phaseConfU2, phaseConfL2, coh2, cohConfU2, cohConfL2] = correctPhaseCoh(phase,...
            phase_confU, phase_confL, coh, coh_conf, rateadjust_kappa);
...
filename = [rootFolder filesep 'globalHalfPRs_area2motion.mat']; %#ok<*UNRCH>
if fullRun

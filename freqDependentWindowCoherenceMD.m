...
function [freq, coh, phase, conf, phaseCu, phaseCl] = freqDependentWindowCoherenceMD(spk1, spk2, dt, spk3, opt)
...
while true
  ...
  params.fpass = [startFreq/opt.freqfactor startFreq];
  if params.fpass(2) < 0.005
    break % We are not interested in smaller frequencies, so break and reduce computation time and potential errors
  end

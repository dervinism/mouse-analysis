...
for dbCount = dbEntries_cLocal % Loop through db entries
  ...
  [dbStruct, ~, ~, entryName, ~, ~, shankIDs,...
    ...
    MUAsAll, spk, muas, ~, ~, muaMetadata] = get_dbStruct(dataStruct, seriesInd, 'negative');
  ...
  for sca = 1:numel(series_ca{dbCount}) % Loop through comparison areas
    ...
    if ~overwrite && isfield(dataStruct, 'seriesData_ca_muas_negative')
      dataString = [animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name)];
      if isfield(dataStruct.seriesData_ca_muas_negative, dataString)
        continue
      end
    end
    
    dbStruct2 = dataStruct.seriesData_negative.([animal '_s' num2str(sca_name)]);
    ...
    dbStruct_ca = dataStruct.seriesData_ca_negative.([animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name)]);
    ...
    if ~isempty(fileList)
      ...
      dataString = ['dataStruct.seriesData_ca_muas_negative.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.phaseCoh = phaseCoh;'];
      eval(dataString);
      dataString = ['dataStruct.seriesData_ca_muas_negative.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.units = muas;'];
      eval(dataString);
      dataString = ['dataStruct.seriesData_ca_muas_negative.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.unitMetadata = muaMetadata;'];
      eval(dataString);
      dataString = ['dataStruct.seriesData_ca_muas_negative.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.popData.phaseCoh = phaseCohDB;'];
      eval(dataString);
      delete([entryName '_*.mat']);
    else
      dataString = ['dataStruct.seriesData_ca_muas_negative.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.phaseCoh = [];'];
      eval(dataString);
      dataString = ['dataStruct.seriesData_ca_muas_negative.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.units = [];'];
      eval(dataString);
      dataString = ['dataStruct.seriesData_ca_muas_negative.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.unitMetadata = [];'];
      eval(dataString);
      dataString = ['dataStruct.seriesData_ca_muas_negative.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.popData.phaseCoh = [];'];
      eval(dataString);
    end

% Run this script to perform analyses comparing LFP ripple rate and pupil area data.
...
% PLOT THE DATA
  phase = bestUnwrap(dbStruct.lfpphaseCohData.rippleRate{ch{dbCount}}.phase);
  phase(isnan(dbStruct.lfpphaseCohData.rippleRate{ch{dbCount}}.phase_confU)) = NaN;
  phase(isnan(dbStruct.lfpphaseCohData.rippleRate{ch{dbCount}}.phase_confL)) = NaN;
  phase(isnan(dbStruct.lfpphaseCohData.rippleRate{ch{dbCount}}.coh_conf)) = NaN;
  freq = dbStruct.lfpphaseCohData.rippleRate{ch{dbCount}}.freq;
...
title(['Ripple rate vs pupil area size.' ' Significant recordings: ' num2str(recSignificant) '/' num2str(recCount)]);
set(figH, 'Name','Ripple rate vs pupil area size');
...
figFileName = [dataDir filesep lfp2pupilDir filesep 'rippleRate2pupil'];

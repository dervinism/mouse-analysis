...
% analyses for area comparisons (within positive subpopulation only).
...
if (~isempty(dbEntries_ca) && dbEntries_ca(1) == inf) ||...
  ...
  dbEntries_ca_positive = 1:numel(fnsData_ca);
else
  dbEntries_ca_positive = dbEntries_ca;
end
for dbCount = dbEntries_ca_positive % Loop through db entries
  dbStruct = dataStruct.seriesData_ca_positive.(fnsData_ca{dbCount});
  ...
  figsubdirname = ['positive' filesep entryName1 'v' entryName2];
  ...
  % Save data
  dataString = ['dataStruct.seriesData_ca_positive.' entryName '.popData.phaseCohHalves = phaseCohHalves;'];
  eval(dataString);
  dataString = ['dataStruct.seriesData_ca_positive.' entryName '.popData.phaseCohHalvesCorr = phaseCohHalvesCorr;'];
  eval(dataString);
end


%% CORRELATION ANALYSES FOR ANIMAL
phaseCohHalves = dataStruct.popData_ca_positive.phaseCohHalves;
if ~isempty(phaseCohHalves.half1.phase)
  ...
  close(figPhase); close(figCoherence);
  dataStruct.popData_ca_positive.phaseCohHalves = phaseCohHalves;

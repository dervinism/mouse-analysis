...
% analyses for positive unit and negative population spiking rate data
...
for dbCount = dbEntriesLocal % Loop through db entries
  ...
  dbCountNegative = find(ismember(fieldnames(dataStruct.seriesData_negative), fnsData{dbCount}));
  ...
  [dbStruct, repository, ~, entryName, ~, ~, shankIDs,...
    ...
    MUAsAll, spkDB, spkDB_units] = get_dbStruct(dataStruct, dbCountNegative, 'negative');
  ...
  if ~isempty(fileList)
    ...
    dataString = ['dataStruct.seriesDataLaminar_negative.' entryName '.shankData.' shankIDs{1} '.phaseCohDeep = phaseCohDeep;'];
    eval(dataString);
    dataString = ['dataStruct.seriesDataLaminar_negative.' entryName '.shankData.' shankIDs{1} '.phaseCohSuperficial = phaseCohSuperficial;'];
    eval(dataString);

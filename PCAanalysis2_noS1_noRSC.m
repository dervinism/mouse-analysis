...
for dbCount = dbStart:numel(fnsData)
  ...
  elseif numel(seriesName) == 15 &&...
      (strcmpi(seriesName(15), '1') || strcmpi(seriesName(15), '3') || strcmpi(seriesName(15), '4') ||...
      strcmpi(seriesName(15), '5') || strcmpi(seriesName(15), '7'))
    continue
  ...
  if dbCount < numel(fnsData) - 2
    seriesNameNext3 = seriesFromEntry(dbStruct.db(dbCount+3).entryName);
  end
  if dbCount == numel(fnsData) || ~strcmpi(seriesName(1:14), seriesNameNext(1:14)) ||...
      ...
      (dbCount == numel(fnsData)-2 && (area == 5 || area == 10)) || ((area == 5 || area == 10) && ~strcmpi(seriesName(1:14), seriesNameNext3(1:14)))
    ...
    for iArea = 1:numel(areasOI)
      ...
      dataString = ['dataStruct.seriesData.' entryNames{iArea} '.pcaData2_noS1_noRSC = pcaData;'];
      eval(dataString);

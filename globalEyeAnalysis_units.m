...
drawPhaseHistos = [true false false];
statsPhaseProfiles = false;
drawPhaseSpans = false;
drawBetaCorrelations = false;
drawPhase2phaseCorrelations = false;
drawPhase2coherenceCorrelations = false;
        ...
        for iCondPlusAll = condLoop % Loop through the main and pooled conditions
          ...
          for sh = 1:numel(shankIDs)
            ...
            for u = 1:numel(units)
              ...
              coh = shankStruct.pupil.unitData{iU}.coh;
              cohConf = shankStruct.pupil.unitData{iU}.coh_conf;
              rateadjust_kappa = shankStruct.pupil.unitData{iU}.rateadjust_kappa;
              phase = shankStruct.pupil.unitData{iU}.phase;
              phaseConfU = shankStruct.pupil.unitData{iU}.phase_confU;
              phaseConfL = shankStruct.pupil.unitData{iU}.phase_confL;
...
%                 phaseConfU, phaseConfL, coh, cohConf, rateadjust_kappa);
              freq = shankStruct.pupil.unitData{iU}.freq;
              %assert(numel(phase) == numel(freq));

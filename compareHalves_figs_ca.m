...
for dbCount = dbEntries_caLocal % Loop through db entries
  dbStruct = dataStruct.seriesData_ca.(fnsData_ca{dbCount});
  ...
  figsubdirname = [entryName1 'v' entryName2];
  ...
  % Save data
  dataString = ['dataStruct.seriesData_ca.' entryName '.popData.phaseCohHalves = phaseCohHalves;'];
  eval(dataString);
  dataString = ['dataStruct.seriesData_ca.' entryName '.popData.phaseCohHalvesCorr = phaseCohHalvesCorr;'];
  eval(dataString);
end


%% CORRELATION ANALYSES FOR ANIMAL
phaseCohHalves = dataStruct.popData_ca.phaseCohHalves;
if ~isempty(phaseCohHalves.half1.phase)
  ...
  close(figPhase); close(figCoherence);
  dataStruct.popData_ca.phaseCohHalves = phaseCohHalves;

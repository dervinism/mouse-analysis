% Run this script to perform analyses comparing PR and total movement data.
...
if strcmp(repository,'all')
  if strcmp(subpop, 'all')
    mainFolder = [dataDir filesep area2motionDir filesep PRsFolder];
  elseif strcmp(subpop, 'positive')
    mainFolder = [dataDir filesep area2motionDir_positive filesep PRsFolder];
  elseif strcmp(subpop, 'negative')
    mainFolder = [dataDir filesep area2motionDir_negative filesep PRsFolder];
  end
  animals = animalsOI;
  xLim = freqLimUOL;
elseif strcmp(repository,'uol')
  if strcmp(subpop, 'all')
    mainFolder = [dataDir filesep area2motionDir_uol filesep PRsFolder];
  elseif strcmp(subpop, 'positive')
    mainFolder = [dataDir filesep area2motionDir_uol_positive filesep PRsFolder];
  elseif strcmp(subpop, 'negative')
    mainFolder = [dataDir filesep area2motionDir_uol_negative filesep PRsFolder];
  end
  animals = animalsUOLOI;
  xLim = freqLimUOL;
elseif strcmp(repository,'allensdk')
  if strcmp(subpop, 'all')
    mainFolder = [dataDir filesep area2motionDir_allensdk filesep PRsFolder];
  elseif strcmp(subpop, 'positive')
    mainFolder = [dataDir filesep area2motionDir_allensdk_positive filesep PRsFolder];
  elseif strcmp(subpop, 'negative')
    mainFolder = [dataDir filesep area2motionDir_allensdk_negative filesep PRsFolder];
  end
    ...
    for dbCount = 1:numel(fnsData) % Loop through database entries
      ...
      for iAreaPlusAll = area % Loop through the main and pooled areas
        ...
        for iCondPlusAll = condLoop % Loop through the main and pooled conditions
          ...
          coh = dbStruct.popData.motion.popData.coh;
          coh_conf = dbStruct.popData.motion.popData.coh_conf;
          rateadjust_kappa = dbStruct.popData.motion.popData.rateadjust_kappa;
          phase = dbStruct.popData.motion.popData.phase;
          phase_confU = dbStruct.popData.motion.popData.phase_confU;
          phase_confL = dbStruct.popData.motion.popData.phase_confL;
...
filename = [mainFolder filesep 'globalPRs_area2motion.mat'];
if fullRun
...
if drawPhaseProfiles(1)
  figFileName = '%sVsMotion_%s_phase';
  options = struct();
  options.figTitle = 'Population rate phase comparisons: %sVsMotion %s';
...
if drawPhaseProfiles(2)
  ...
  options = struct();
  options.figTitle = 'Population rate mean phase comparisons: %sVsMotion';
  options.figSize = figSize;
  options.mainFolder = mainFolder;
  options.phaseFrequencyProfilesSubfolder = phaseFrequencyProfilesSubfolder;
  options.freqLim = [xLim(1) 2];
...
if drawCohProfiles(1)
  figFileName = '%sVsMotion_%s_coherence';
  options = struct();
  options.figTitle = 'Population rate coherence comparisons: %sVsMotion %s';
  options.figSize = figSize;
  options.mainFolder = mainFolder;
...
if drawCohProfiles(2)
  figFileName = 'Means_only__%sVsMotion';
  options = struct();
  options.figTitle = 'Population rate mean coherence comparisons: %sVsMotion';
  options.figSize = figSize;
  options.mainFolder = mainFolder;
  options.cohFrequencyProfilesSubfolder = coherenceFrequencyProfilesSubfolder;
  options.freqLim = [xLim(1) 2];

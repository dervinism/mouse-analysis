...
% of the same recording for area comparisons (within negative subpopulation
% only).
...
for dbCount = dbEntries_caLocal % Loop through db entries
  ...
  dbStruct = dataStruct.seriesData_ca_muas_negative.(fnsData_ca{dbCount});
  if isempty(dbStruct)
    continue
  end
  ...
  % Save data
  dataString = ['dataStruct.seriesData_ca_muas_negative.' fnsData_ca{dbCount} '.popData.phaseCohHalves = phaseCohHalvesDB;'];
  eval(dataString);
  dataStruct.popData_ca_muas_negative.phaseCohHalves = phaseCohHalves;

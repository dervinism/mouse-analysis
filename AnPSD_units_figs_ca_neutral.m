...
% different area (within uncorrelated subpopulation only).
...
if (~isempty(dbEntries) && dbEntries(1) == inf) ||...
  ...
  dbEntries_neutral = 1:numel(fnsData);
else
  dbEntries_neutral = dbEntries;
end
for dbCount = dbEntries_neutral % Loop through db entries
  
  % Load the contents of dbStruct
  [~, ~, ~, entryName, ~, ~, shankIDs,...
    ~, ~, ~, ~, srData, ~, ~, FOI] = get_dbStruct(dataStruct, dbCount, 'neutral');
  ...
  for sca = 1:numel(fnsData_ca) % Loop through comparison areas
    ...
    dbStruct_ca = dataStruct.seriesData_ca_neutral.(sca_name);
    ...
    figsubdirname = ['neutral' filesep sca_name1 'v' sca_name2];

...
% dataDir\caDir\unitsFolder\phaseCorrelationsSubfolder\globalHalfUnits_ca.mat or
%   dataDir\caDir\unitsFolder\phaseCorrelationsSubfolder\globalHalfUnits_ca_reverse.mat or
%   dataDir\caDir\qualityUnitsFolder\phaseCorrelationsSubfolder\globalHalfUnits_ca_quality.mat or
%   dataDir\caDir\qualityUnitsFolder\phaseCorrelationsSubfolder\globalHalfUnits_ca_reverse_quality.mat
...
%   in dataDir\caDir\unitsFolder\phaseCorrelationsSubfolder or
%   dataDir\caDir\qualityUnitsFolder\phaseCorrelationsSubfolder.
        ...
        for iCondPlusAll = condLoop % Loop through the main and pooled conditions
          ...
          for sh = 1:numel(dbStruct_ca.shankData)
            ...
            for u = 1:numel(units)
              ...
              coh1 = shankStruct.phaseCoh{iU}.coh_halves(1,:);
              cohConf1 = shankStruct.phaseCoh{iU}.coh_conf_halves(1,:);
              rateadjust_kappa1 = shankStruct.phaseCoh{iU}.rateadjust_kappa_halves(1,:);
              phase1 = shankStruct.phaseCoh{iU}.phase_halves(1,:);
              phaseConfU1 = shankStruct.phaseCoh{iU}.phase_conf_halves(1,:);
              phaseConfL1 = shankStruct.phaseCoh{iU}.phase_conf_halves(2,:);
...
%                 phaseConfU1, phaseConfL1, coh1, cohConf1, rateadjust_kappa1);
              ...
              if ~sum(~isnan(freq)) && sum(~isnan(coh1))
                freq = shankStruct.phaseCoh{iU}.coh_halves_freq;
              end
              ...
              coh2 = shankStruct.phaseCoh{iU}.coh_halves(2,:);
              cohConf2 = shankStruct.phaseCoh{iU}.coh_conf_halves(2,:);
              rateadjust_kappa2 = shankStruct.phaseCoh{iU}.rateadjust_kappa_halves(2,:);
              phase2 = shankStruct.phaseCoh{iU}.phase_halves(2,:);
              phaseConfU2 = shankStruct.phaseCoh{iU}.phase_conf_halves(3,:);
              phaseConfL2 = shankStruct.phaseCoh{iU}.phase_conf_halves(4,:);
...
if reverse
  if qualityCheck %#ok<*UNRCH>
    filename = [fileparts(mainFolderPhase) filesep 'globalHalfUnits_ca_reverse_quality.mat'];
  else
    filename = [fileparts(mainFolderPhase) filesep 'globalHalfUnits_ca_reverse.mat'];
  end
else
  if qualityCheck
    filename = [fileparts(mainFolderPhase) filesep 'globalHalfUnits_ca_quality.mat'];
  else
    filename = [fileparts(mainFolderPhase) filesep 'globalHalfUnits_ca.mat'];
  end
end
if fullRun

...
% of the same recording for area comparisons (within uncorrelated subpopulation
% only).
...
for dbCount = dbEntries_caLocal % Loop through db entries
  ...
  [~, ~, ~, ~, ~, ~, ~, ~, ~, ~, ~, ~, ~, ~, FOI] = get_dbStruct(dataStruct, find(endsWith(fnsData,seriesName1)), 'neutral');
  ...
  % Save data
  dataString = ['dataStruct.seriesData_ca_neutral.' fnsData_ca{dbCount} '.popData.phaseCohHalves = phaseCohHalvesDB;'];
  eval(dataString);

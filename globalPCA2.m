...
for a = 1:numel(animals)
  ...
  for dbCount = 1:numel(fnsData)
    ...
    for i = 1:2 % Repeat twice to deal with non-specific condition assignment
      ...
      if area == 2
        VB_PCs_v = torow(dbStruct.pcaData2.explained_area(1:nPCs2plot));
        VB_PCs = cumsum(torow(dbStruct.pcaData2.explained_area(1:nPCs2plot)));
      elseif area == 5
        CA1_PCs_v = torow(dbStruct.pcaData2.explained_area(1:nPCs2plot));
        CA1_PCs = cumsum(torow(dbStruct.pcaData2.explained_area(1:nPCs2plot)));
        for iPC = 1:nPCs2plot

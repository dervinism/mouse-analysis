...
% different area (within negative subpopulation only).
...
if (~isempty(dbEntries) && dbEntries(1) == inf) ||...
  ...
  dbEntries_negative = 1:numel(fnsData);
else
  dbEntries_negative = dbEntries;
end
for dbCount = dbEntries_negative % Loop through db entries
  
  % Load the contents of dbStruct
  [~, ~, ~, entryName, ~, ~, shankIDs,...
    ~, ~, ~, ~, srData, ~, ~, FOI] = get_dbStruct(dataStruct, dbCount, 'negative');
  ...
  for sca = 1:numel(fnsData_ca) % Loop through comparison areas
    ...
    dbStruct_ca = dataStruct.seriesData_ca_negative.(sca_name);
    ...
    figsubdirname = ['negative' filesep sca_name1 'v' sca_name2];

...
% for area-to-motion comparisons.
%
% The following data files are produced:
% dataDir\area2motion\unitsFolder\phaseLocationProfilesSubfolder\globalUnitsLocs_area2motion.mat or
%   dataDir\area2motion\qualityUnitsFolder\phaseLocationProfilesSubfolder\globalUnitsLocs_area2motion_quality.mat.
...
%   dataDir\area2motion\unitsFolder\phaseLocationProfilesSubfolder or
%   dataDir\area2motion\qualityUnitsFolder\phaseLocationProfilesSubfolder.
...
if qualityCheck
  mainFolder = [dataDir filesep area2motionDir filesep qualityUnitsFolder filesep phaseLocationProfilesSubfolder];
else
  mainFolder = [dataDir filesep area2motionDir filesep unitsFolder filesep phaseLocationProfilesSubfolder];
end
    ...
    for dbCount = 1:numel(fnsData) % Loop through database entries
      ...
      for iAreaPlusAll = area % Loop through the main and pooled areas
        ...
        for iCondPlusAll = condLoop % Loop through the main and pooled conditions
          ...
          phase = dbStruct.popData.motion.unitData.phaseFOI(qualityUnitInd,:);
          ...
          coh = dbStruct.popData.motion.unitData.cohFOI(qualityUnitInd,:);
...
if qualityCheck
  filename = [mainFolder filesep 'globalUnitsLocs_area2motion_quality.mat']; %#ok<*UNRCH>
else
  filename = [mainFolder filesep 'globalUnitsLocs_area2motion.mat'];
end
if fullRun
  ...
  for iArea = 1:numel(areas)
    if ~isempty(areaPhaseFOIindividual{iCond}{iArea})
      ...
      [slicePhaseFig, rFOI{iCond}{iArea}, pvalFOI{iCond}{iArea}, nFOI{iCond}{iArea}, distributionStats{iCond}{iArea}] = phaseLocationProfile(...
        ...
        individualGraphs, figPath, true, 'MOTION', 'VsMotion');
      close all;
      
      % Summary graphs
      if summaryGraphs
        sbPhase = phaseLocationProfileSummary(areaPhaseFOIindividual{iCond}{iArea}, areaLocFOIindividual{iCond}{iArea},...
          FOI, rFOI{iCond}{iArea}, pvalFOI{iCond}{iArea}, nFOI{iCond}{iArea}, areas{iArea}, conditions{iCond}, figPath, 'MOTION', 'VsMotion');
        close(sbPhase);
      end
    end
  end

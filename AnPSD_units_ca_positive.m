...
for dbCount = dbEntries_cLocal % Loop through db entries (area 1)
  ...
  [dbStruct, ~, ~, entryName, ~, ~, shankIDs,...
    ...
    MUAsAll] = get_dbStruct(dataStruct, seriesInd, 'positive');
  ...
  % Test if the analysis data already exists
    if ~overwrite && isfield(dataStruct, 'seriesData_ca_positive')
      dataString = [animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name)];
      if isfield(dataStruct.seriesData_ca_positive, dataString)
        if isfield(dataStruct.seriesData_ca_positive.(dataString), 'shankData')
          continue
        end
      end
    end
    
    if ~isfield(dataStruct.seriesData_positive, [animal '_s' num2str(sca_name)])
      continue
    end
    dbStruct_ca = dataStruct.seriesData_positive.([animal '_s' num2str(sca_name)]);
    ...
    for sh = 1:numel(shankIDs_ca)
      ...
      for u = 1:numel(units)
        ...
        if numel(PR2) > numel(spkOI_ca)
          PR2 = PR2 + [spkOI_ca zeros(1,numel(PR2)-numel(spkOI_ca))]; % pure units PR for the second series
    ...
    for sh = 1:numel(shankIDs) % Loop through shanks
      ...
      if ~isempty(fileList)
        ...
        dataString = ['dataStruct.seriesData_ca_positive.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.shankData.' shankIDs{sh} '.phaseCoh = phaseCoh;'];
        eval(dataString);
        dataString = ['dataStruct.seriesData_ca_positive.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.shankData.' shankIDs{sh} '.shankID = sh;'];
        eval(dataString);
        dataString = ['dataStruct.seriesData_ca_positive.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.shankData.' shankIDs{sh} '.units = units;'];
        eval(dataString);
        dataString = ['dataStruct.seriesData_ca_positive.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.shankData.' shankIDs{sh} '.unitMetadata = unitMetadata;'];
        eval(dataString);
        dataString = ['dataStruct.seriesData_ca_positive.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.popData.phaseCoh = phaseCohDB;'];
        eval(dataString);
        ...
        dataString = ['dataStruct.seriesData_ca_positive.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.shankData.' shankIDs{sh} '.phaseCoh = {};'];
        eval(dataString);
        dataString = ['dataStruct.seriesData_ca_positive.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.shankData.' shankIDs{sh} '.shankID = sh;'];
        eval(dataString);
        dataString = ['dataStruct.seriesData_ca_positive.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.shankData.' shankIDs{sh} '.units = [];'];
        eval(dataString);
        dataString = ['dataStruct.seriesData_ca_positive.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.shankData.' shankIDs{sh} '.unitMetadata = [];'];
        eval(dataString);
        dataString = ['dataStruct.seriesData_ca_positive.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.popData.phaseCoh = phaseCohDB;'];
        eval(dataString);
        if intermediateSaving
    ...
    % Save
    dataString = ['dataStruct.seriesData_ca_positive.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.popData.phaseCohPop = popData_ca;'];
    eval(dataString);
    dataString = ['dataStruct.seriesData_ca_positive.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.popData.phaseCohPopUnitOnly = popData_ca2;'];
    eval(dataString);
    if intermediateSaving

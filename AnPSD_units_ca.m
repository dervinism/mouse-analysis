...
for dbCount = dbEntries_cLocal % Loop through db entries (area 1)
  ...
  [dbStruct, ~, ~, entryName, ~, ~, shankIDs,...
    ...
    MUAsAll] = get_dbStruct(dataStruct, find(endsWith(fnsData,seriesName1)));
  ...
  % Test if the analysis data already exists
    if ~overwrite && isfield(dataStruct, 'seriesData_ca')
      dataString = [animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name)];
      if isfield(dataStruct.seriesData_ca, dataString)
        if isfield(dataStruct.seriesData_ca.(dataString), 'shankData')
          continue
        end
      end
    end
    
    dbStruct_ca = dataStruct.seriesData.([animal '_s' num2str(sca_name)]);
    ...
    for sh = 1:numel(shankIDs) % Loop through shanks
      ...
      if ~isempty(fileList)
        ...
        dataString = ['dataStruct.seriesData_ca.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.shankData.' shankIDs{sh} '.phaseCoh = phaseCoh;'];
        eval(dataString);
        dataString = ['dataStruct.seriesData_ca.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.shankData.' shankIDs{sh} '.shankID = sh;'];
        eval(dataString);
        dataString = ['dataStruct.seriesData_ca.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.shankData.' shankIDs{sh} '.units = units;'];
        eval(dataString);
        dataString = ['dataStruct.seriesData_ca.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.shankData.' shankIDs{sh} '.unitMetadata = unitMetadata;'];
        eval(dataString);
        dataString = ['dataStruct.seriesData_ca.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.popData.phaseCoh = phaseCohDB;'];
        eval(dataString);
        ...
        dataString = ['dataStruct.seriesData_ca.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.shankData.' shankIDs{sh} '.phaseCoh = {};'];
        eval(dataString);
        dataString = ['dataStruct.seriesData_ca.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.shankData.' shankIDs{sh} '.shankID = sh;'];
        eval(dataString);
        dataString = ['dataStruct.seriesData_ca.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.shankData.' shankIDs{sh} '.units = [];'];
        eval(dataString);
        dataString = ['dataStruct.seriesData_ca.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.shankData.' shankIDs{sh} '.unitMetadata = [];'];
        eval(dataString);
        dataString = ['dataStruct.seriesData_ca.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.popData.phaseCoh = phaseCohDB;'];
        eval(dataString);
        if intermediateSaving
    ...
    % Save
    dataString = ['dataStruct.seriesData_ca.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.popData.phaseCohPop = popData_ca;'];
    eval(dataString);
    dataString = ['dataStruct.seriesData_ca.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.popData.phaseCohPopUnitOnly = popData_ca2;'];
    eval(dataString);
    if intermediateSaving

...
for dbCount = dbStart:numel(fnsData)
  ...
  % TEST FOR EYE DATA
  if isfield(dbStruct, 'lfpPowerData') && ~isfield(dbStruct.lfpPowerData, 'motionInterpFilt')
    continue
  end
  ...
  if dbCount == numel(fnsData) || ~strcmpi(seriesName(1:14), seriesNameNext(1:14)) ||...
    ...
    motion = torow(dbStruct.lfpPowerData.motionInterpFilt);
    motionTimes = torow(dbStruct.lfpPowerData.motionInterpTimes);
    ...
    if params.srData ~= dssrLFPinit
      motion = torow(interp1(motionTimes, motion, PCs2motionTimes));
    end
    
    
    % ADD AROUSAL DATA
    neuralActivity = neuralActivity(:,iPCs2motionStart:iPCs2motionEnd);
    neuralActivity = [neuralActivity; motion];
    neuralActivity(isnan(neuralActivity)) = 0;
    areas = [areas; 0];
    ...
    for iArea = 1:numel(areasOI)
      ...
      [rPCs2lfpPearson_area, pvalPCs2lfpPearson_area] = corrMulti(LFPs(iArea,iPCs2motionStart:iPCs2motionEnd), PCs, 'Pearson');
      [rPCs2lfpSpearman_area, pvalPCs2lfpSpearman_area] = corrMulti(LFPs(iArea,iPCs2motionStart:iPCs2motionEnd), PCs, 'Spearman');
    
    
      % CORRELATE PCs WITH PR
      [rPCs2prPearson_area, pvalPCs2prPearson_area] = corrMulti(PRs(iArea,iPCs2motionStart:iPCs2motionEnd), PCs, 'Pearson');
      [rPCs2prSpearman_area, pvalPCs2prSpearman_area] = corrMulti(PRs(iArea,iPCs2motionStart:iPCs2motionEnd), PCs, 'Spearman');
      ...
      dataString = ['dataStruct.seriesData.' entryNames{iArea} '.pcaData3_motion = pcaData;'];
      eval(dataString);

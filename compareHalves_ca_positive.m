...
% relevant data that was already produced in AnPSD_units_ca_positive.
...
for dbCount = dbEntries_caLocal % Loop through db entries
  ...
  [~, ~, ~, ~, ~, ~, ~, ~, ~, ~, ~, ~, ~, ~, FOI] = get_dbStruct(dataStruct, find(endsWith(fnsData,seriesName1)), 'positive');
  ...
  % Save data
  dataString = ['dataStruct.seriesData_ca_positive.' fnsData_ca{dbCount} '.popData.phaseCohHalves = phaseCohHalvesDB;'];
  eval(dataString);

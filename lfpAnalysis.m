...
for dbCount = dbEntries % Loop though db entries
  ...
  [seriesName, animal] = seriesFromEntry(entryName);
  entryNameEye = [animal '_s' seriesName(1:14)];
  commonPeriod = combinePeriods(periodLFP, dataStruct.eyeData.(entryNameEye).period, srData);
  if isempty(commonPeriod)
    continue
  end
  [areaInterpFilt, interpTimes, areaInd] = pupilFilt(eyeDataDB, dssrLFPfinal, lfpTimes, 2*dssrLFPfinal, commonPeriod, dssrLFPfinal);
  ...
  for iCh = 1:numel(chOIDB)
    ...
    % LOAD LFP MEASURES
    rippleRate = dbStruct.lfpPowerData.rippleRate{iCh}(areaInd);
    meanRippleRate = dbStruct.lfpPowerData.meanRippleRate{iCh};
    theta2deltaRatio = dbStruct.lfpPowerData.theta2deltaRatio{iCh}(areaInd);
    slowPower = dbStruct.lfpPowerData.slowPower{iCh}(areaInd);
    fastPower = dbStruct.lfpPowerData.fastPower{iCh}(areaInd);
    ultraFastPower = dbStruct.lfpPowerData.ultraFastPower{iCh}(areaInd);
    ...
    [lfpPhaseCohDB.rippleRate{iCh}.corr(1,1), lfpPhaseCohDB.rippleRate{iCh}.corr(1,2)] = corrSimple(areaInterpFilt, rippleRate, 'Pearson');
    [lfpPhaseCohDB.rippleRate{iCh}.corr(2,1), lfpPhaseCohDB.rippleRate{iCh}.corr(2,2)] = corrSimple(areaInterpFilt, rippleRate, 'Spearman');
    lfpPhaseCohDB.t2dRatio{iCh}.corr = zeros(2,2);
    [lfpPhaseCohDB.t2dRatio{iCh}.corr(1,1), lfpPhaseCohDB.t2dRatio{iCh}.corr(1,2)] = corrSimple(areaInterpFilt, theta2deltaRatio, 'Pearson');
    [lfpPhaseCohDB.t2dRatio{iCh}.corr(2,1), lfpPhaseCohDB.t2dRatio{iCh}.corr(2,2)] = corrSimple(areaInterpFilt, theta2deltaRatio, 'Spearman');
    lfpPhaseCohDB.slowPower{iCh}.corr = zeros(2,2);
    [lfpPhaseCohDB.slowPower{iCh}.corr(1,1), lfpPhaseCohDB.slowPower{iCh}.corr(1,2)] = corrSimple(areaInterpFilt, slowPower, 'Pearson');
    [lfpPhaseCohDB.slowPower{iCh}.corr(2,1), lfpPhaseCohDB.slowPower{iCh}.corr(2,2)] = corrSimple(areaInterpFilt, slowPower, 'Spearman');
    lfpPhaseCohDB.fastPower{iCh}.corr = zeros(2,2);
    [lfpPhaseCohDB.fastPower{iCh}.corr(1,1), lfpPhaseCohDB.fastPower{iCh}.corr(1,2)] = corrSimple(areaInterpFilt, fastPower, 'Pearson');
    [lfpPhaseCohDB.fastPower{iCh}.corr(2,1), lfpPhaseCohDB.fastPower{iCh}.corr(2,2)] = corrSimple(areaInterpFilt, fastPower, 'Spearman');
    lfpPhaseCohDB.ultraFastPower{iCh}.corr = zeros(2,2);
    [lfpPhaseCohDB.ultraFastPower{iCh}.corr(1,1), lfpPhaseCohDB.ultraFastPower{iCh}.corr(1,2)] = corrSimple(areaInterpFilt, ultraFastPower, 'Pearson');
    [lfpPhaseCohDB.ultraFastPower{iCh}.corr(2,1), lfpPhaseCohDB.ultraFastPower{iCh}.corr(2,2)] = corrSimple(areaInterpFilt, ultraFastPower, 'Spearman');
    
    % Plot graphs
    %figure; plot(areaInterpFilt,rippleRate, 'r.', 'MarkerSize',1)
    %figure; plot(areaInterpFilt,theta2deltaRatio, 'r.', 'MarkerSize',1)
    %figure; plot(areaInterpFilt,slowPower, 'r.', 'MarkerSize',1)
    %figure; plot(areaInterpFilt,fastPower, 'r.', 'MarkerSize',1)
    %figure; plot(areaInterpFilt,ultraFastPower, 'r.', 'MarkerSize',1)
    figs{iCh} = figure('Visible', visibility); %#ok<*SAGROW>
    plot(interpTimes,areaInterpFilt./mean(areaInterpFilt), 'LineWidth',2); hold on;
    ...
    legend('Pupil area','Ripple rate','Theta2delta ratio','Slow power','Fast power','Ultra fast power','LFP saturations')
    xlabel('Time (s)')
    ...
    figName = [figsubdirname filesep entryName '_LFP_variousSignals_v_pupil_' 'ch' num2str(chOIDB(iCh))];
    ...
    [lfpPhaseCohDB.rippleRate{iCh}.freq, lfpPhaseCohDB.rippleRate{iCh}.coh, lfpPhaseCohDB.rippleRate{iCh}.phase,...
      ...
      lfpPhaseCohDB.rippleRate{iCh}.mean_var, lfpPhaseCohDB.rippleRate{iCh}.mean_var_timeBin] = phaseCohLFP(areaInterpFilt, rippleRate,...
    ...
    [lfpPhaseCohDB.t2dRatio{iCh}.freq, lfpPhaseCohDB.t2dRatio{iCh}.coh, lfpPhaseCohDB.t2dRatio{iCh}.phase,...
      ...
      lfpPhaseCohDB.t2dRatio{iCh}.mean_var, lfpPhaseCohDB.t2dRatio{iCh}.mean_var_timeBin] = phaseCohLFP(areaInterpFilt, theta2deltaRatio,...
      dssrLFPfinal, optLFP, 1e2);
    [lfpPhaseCohDB.slowPower{iCh}.freq, lfpPhaseCohDB.slowPower{iCh}.coh, lfpPhaseCohDB.slowPower{iCh}.phase,...
      ...
      lfpPhaseCohDB.slowPower{iCh}.mean_var, lfpPhaseCohDB.slowPower{iCh}.mean_var_timeBin] = phaseCohLFP(areaInterpFilt, slowPower,...
      dssrLFPfinal, optLFP, 1e2);
    [lfpPhaseCohDB.fastPower{iCh}.freq, lfpPhaseCohDB.fastPower{iCh}.coh, lfpPhaseCohDB.fastPower{iCh}.phase,...
      ...
      lfpPhaseCohDB.fastPower{iCh}.mean_var, lfpPhaseCohDB.fastPower{iCh}.mean_var_timeBin] = phaseCohLFP(areaInterpFilt, fastPower,...
      dssrLFPfinal, optLFP, 1e2);
    [lfpPhaseCohDB.ultraFastPower{iCh}.freq, lfpPhaseCohDB.ultraFastPower{iCh}.coh, lfpPhaseCohDB.ultraFastPower{iCh}.phase,...
      ...
      lfpPhaseCohDB.ultraFastPower{iCh}.mean_var, lfpPhaseCohDB.ultraFastPower{iCh}.mean_var_timeBin] = phaseCohLFP(areaInterpFilt, ultraFastPower,...
      dssrLFPfinal, optLFP, 1e2);
    ...
    if sum(~isnan(lfpPhaseCohDB.rippleRate{iCh}.freq))
      ...
      figName = [figsubdirname filesep entryName '_LFP_rippleRate_v_pupil' '_PSDpolar_' 'ch' num2str(chOIDB(iCh))];
      set(figTmp, 'Name',figName);
      hgsave(figTmp, figName);
      close(figTmp);
      halfPhaseCohFig(lfpPhaseCohDB.rippleRate{iCh},...
        [figsubdirname filesep entryName '_LFP_rippleRate_v_pupil' '_phaseCohHalves_' 'ch' num2str(chOIDB(iCh))], 'Phase', 'Coherence', visibility);
    end
    ...
    if sum(~isnan(lfpPhaseCohDB.t2dRatio{iCh}.freq))
      ...
      figName = [figsubdirname filesep entryName '_LFP_t2dRatio_v_pupil' '_PSDpolar_' 'ch' num2str(chOIDB(iCh))];
      set(figTmp, 'Name',figName);
      hgsave(figTmp, figName);
      close(figTmp);
      halfPhaseCohFig(lfpPhaseCohDB.t2dRatio{iCh},...
        [figsubdirname filesep entryName '_LFP_t2dRatio_v_pupil' '_phaseCohHalves_' 'ch' num2str(chOIDB(iCh))], 'Phase', 'Coherence', visibility);
    end
    ...
    if sum(~isnan(lfpPhaseCohDB.slowPower{iCh}.freq))
      ...
      figName = [figsubdirname filesep entryName '_LFP_slowPower_v_pupil' '_PSDpolar_' 'ch' num2str(chOIDB(iCh))];
      set(figTmp, 'Name',figName);
      hgsave(figTmp, figName);
      close(figTmp);
      halfPhaseCohFig(lfpPhaseCohDB.slowPower{iCh},...
        [figsubdirname filesep entryName '_LFP_slowPower_v_pupil' '_phaseCohHalves_' 'ch' num2str(chOIDB(iCh))], 'Phase', 'Coherence', visibility);
    end
    ...
    if sum(~isnan(lfpPhaseCohDB.fastPower{iCh}.freq))
      ...
      figName = [figsubdirname filesep entryName '_LFP_fastPower_v_pupil' '_PSDpolar_' 'ch' num2str(chOIDB(iCh))];
      set(figTmp, 'Name',figName);
      hgsave(figTmp, figName);
      close(figTmp);
      halfPhaseCohFig(lfpPhaseCohDB.fastPower{iCh},...
        [figsubdirname filesep entryName '_LFP_fastPower_v_pupil' '_phaseCohHalves_' 'ch' num2str(chOIDB(iCh))], 'Phase', 'Coherence', visibility);
    end
    ...
    if sum(~isnan(lfpPhaseCohDB.ultraFastPower{iCh}.freq))
      ...
      figName = [figsubdirname filesep entryName '_LFP_ultraFastPower_v_pupil' '_PSDpolar_' 'ch' num2str(chOIDB(iCh))];
      set(figTmp, 'Name',figName);
      hgsave(figTmp, figName);
      close(figTmp);
      halfPhaseCohFig(lfpPhaseCohDB.ultraFastPower{iCh},...
        [figsubdirname filesep entryName '_LFP_ultraFastPower_v_pupil' '_phaseCohHalves_' 'ch' num2str(chOIDB(iCh))], 'Phase', 'Coherence', visibility);
    end
  end
  
  % SAVE DATA
  dataString = ['dataStruct.seriesData.' entryName '.lfpPowerData.areaInterpFilt = areaInterpFilt;'];
  eval(dataString);
  dataString = ['dataStruct.seriesData.' entryName '.lfpPowerData.areaInterpTimes = interpTimes;'];
  eval(dataString);
  dataString = ['dataStruct.seriesData.' entryName '.lfpphaseCohData = lfpPhaseCohDB;'];
  eval(dataString);
...
function [freq, coh, phase, coh_conf, phase_confU, phase_confL, coh_halves_freq, coh_halves, coh_conf_halves, phase_halves,...
  phase_conf_halves, stPRgr0, stPR, m, mv1, mv2, mean_var, mean_var_timeBin] = phaseCohLFP(pupilAreaSignal, LFPsignal,...
...
[freq, coh, phase, coh_conf, phase_confU, phase_confL, coh_halves_freq, coh_halves, coh_conf_halves, phase_halves,...
  phase_conf_halves] = phaseCohCalc(pupilAreaSignal, LFPsignal, dssrLFPfinal, optLFP);

% CROSSCORRELATE POPULATION SPIKING AND PUPIL AREA
stPRgr0 = stprCalc(LFPsignal, pupilAreaSignal, stPRwindow);
stPR = stprHalfCalc(LFPsignal, pupilAreaSignal, stPRwindow);

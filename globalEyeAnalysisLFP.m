...
if strcmp(repository,'uol')
  if strcmp(subpop, 'all')
    mainFolder = [dataDir filesep area2pupilDir_uol];
  elseif strcmp(subpop, 'positive')
    mainFolder = [dataDir filesep area2pupilDir_uol_positive];
  elseif strcmp(subpop, 'negative')
    mainFolder = [dataDir filesep area2pupilDir_uol_negative];
  end
  animals = animalsUOLOI;
  xLim = freqLimUOL;
elseif strcmp(repository,'allensdk')
  if strcmp(subpop, 'all')
    mainFolder = [dataDir filesep area2pupilDir_allensdk];
  elseif strcmp(subpop, 'positive')
    mainFolder = [dataDir filesep area2pupilDir_allensdk_positive];
  elseif strcmp(subpop, 'negative')
    mainFolder = [dataDir filesep area2pupilDir_allensdk_negative];
  end
    ...
    for dbCount = 1:numel(fnsData) % Loop through database entries
      ...
      for iAreaPlusAll = area % Loop through the main and pooled areas
        ...
        for iCondPlusAll = condLoop % Loop through the main and pooled conditions
          ...
          freq = dbStruct.lfpData.rippleRate{iCh}.freq;
...
filename = [mainFolder filesep 'globalMUAs_area2pupil.mat'];
if fullRun

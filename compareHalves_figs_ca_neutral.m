...
% analyses for area comparisons (within uncorrelated subpopulation only).
...
fnsData_ca = fieldnames(dataStruct.seriesData_ca_neutral);
if (~isempty(dbEntries_ca) && dbEntries_ca(1) == inf) ||...
    (~isempty(dbEntries_ca) && dbEntries_ca(end) > numel(fnsData_ca))
  dbEntries_ca_neutral = 1:numel(fnsData_ca);
else
  dbEntries_ca_neutral = dbEntries_ca;
end
for dbCount = dbEntries_ca_neutral % Loop through db entries
  dbStruct = dataStruct.seriesData_ca_neutral.(fnsData_ca{dbCount});
  if ~isfield(dbStruct.popData, 'phaseCohHalves')
    continue
  end
  phaseCohHalves = dbStruct.popData.phaseCohHalves;
  entryName = fnsData_ca{dbCount};
  FOI = dataStruct.seriesData_neutral.(fnsData{1}).conf.FOI;
  ...
  figsubdirname = ['neutral' filesep entryName1 'v' entryName2];
  ...
  % Save data
  dataString = ['dataStruct.seriesData_ca_neutral.' entryName '.popData.phaseCohHalves = phaseCohHalves;'];
  eval(dataString);
  dataString = ['dataStruct.seriesData_ca_neutral.' entryName '.popData.phaseCohHalvesCorr = phaseCohHalvesCorr;'];
  eval(dataString);
end


%% CORRELATION ANALYSES FOR ANIMAL
phaseCohHalves = dataStruct.popData_ca_neutral.phaseCohHalves;
if ~isempty(phaseCohHalves.half1.phase)
  ...
  close(figPhase); close(figCoherence);
  dataStruct.popData_ca_neutral.phaseCohHalves = phaseCohHalves;

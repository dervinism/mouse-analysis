...
if qualityCheck
  ...
  if strcmp(subpop, 'all')
    rootFolderPositive = [rootFolderPositive filesep qualityUnitsFolder];
    rootFolderNegative = [rootFolderNegative filesep qualityUnitsFolder];
  end
else
  rootFolder = [rootFolder filesep unitsFolder];
  if strcmp(subpop, 'all')
    rootFolderPositive = [rootFolderPositive filesep unitsFolder];
    rootFolderNegative = [rootFolderNegative filesep unitsFolder];
  end
end
...
if significantUnits
  ...
  if strcmp(subpop, 'all')
    rootFolderPositive = [mainFolderPositive filesep significantUnitsFolder];
    mainFolderPositive = rootFolderPositive;
    rootFolderNegative = [mainFolderNegative filesep significantUnitsFolder];
      ...
      for sh = 1:numel(fieldnames(dbStruct.shankData))
        ...
        if crossValidation % not implemented yet to work with the latest +/- splitting based on phase
          if strcmpi(crossValidationType, '10percent')
            rSpearman = [rSpearman; dbStruct.shankData.(['shank' num2str(sh)]).rSpearman10percent(1,:)'];
            pvalSpearman = [pvalSpearman; dbStruct.shankData.(['shank' num2str(sh)]).pvalSpearman10percent(1,:)'];
          elseif strcmpi(crossValidationType, '25percent')
            rSpearman = [rSpearman; dbStruct.shankData.(['shank' num2str(sh)]).rSpearman25percent(1,:)'];
            pvalSpearman = [pvalSpearman; dbStruct.shankData.(['shank' num2str(sh)]).pvalSpearman25percent(1,:)'];
          elseif strcmpi(crossValidationType, '33percent')
            rSpearman = [rSpearman; dbStruct.shankData.(['shank' num2str(sh)]).rSpearman33percent(1,:)'];
            pvalSpearman = [pvalSpearman; dbStruct.shankData.(['shank' num2str(sh)]).pvalSpearman33percent(1,:)'];
          elseif strcmpi(crossValidationType, '50percent')
            rSpearman = [rSpearman; dbStruct.shankData.(['shank' num2str(sh)]).rSpearman50percent(1,:)'];
            pvalSpearman = [pvalSpearman; dbStruct.shankData.(['shank' num2str(sh)]).pvalSpearman50percent(1,:)'];
          elseif strcmpi(crossValidationType, '10percentBack')
            rSpearman = [rSpearman; dbStruct.shankData.(['shank' num2str(sh)]).rSpearman10percent(end,:)'];
            pvalSpearman = [pvalSpearman; dbStruct.shankData.(['shank' num2str(sh)]).pvalSpearman10percent(end,:)'];
          elseif strcmpi(crossValidationType, '25percentBack')
            rSpearman = [rSpearman; dbStruct.shankData.(['shank' num2str(sh)]).rSpearman25percent(end,:)'];
            pvalSpearman = [pvalSpearman; dbStruct.shankData.(['shank' num2str(sh)]).pvalSpearman25percent(end,:)'];
          elseif strcmpi(crossValidationType, '33percentBack')
            rSpearman = [rSpearman; dbStruct.shankData.(['shank' num2str(sh)]).rSpearman33percent(end,:)'];
            pvalSpearman = [pvalSpearman; dbStruct.shankData.(['shank' num2str(sh)]).pvalSpearman33percent(end,:)'];
          elseif strcmpi(crossValidationType, '50percentBack')
            rSpearman = [rSpearman; dbStruct.shankData.(['shank' num2str(sh)]).rSpearman50percent(end,:)'];
          ...
          elseif iCond == 2
            if ~isfield(dbStruct.shankData.(['shank' num2str(sh)]), 'rSpearman') || ...
              ...
              pvalSpearman = [pvalSpearman; dbStruct.shankData.(['shank' num2str(sh)]).pvalSpearman'];
            end
          end
        end
      end
          ...
          for sh = 1:numel(shankIDs)
            if isempty(spk)
              ...
              if iCond == 1
                ...
                phase = recentrePhase(phase, modeBoundaries(3));
                negativeInd(phase > modeBoundaries(2) & phase <= modeBoundaries(end)) = true;
                rSpearmanSh = spkCoh(dbStruct.shankData.(['shank' num2str(sh)]).pupil.unitData, fRef);
                rSpearmanSh(negativeInd) = -rSpearmanSh(negativeInd); % signed coherence rather than Spearman rho, so remember not to confuse
...
if qualityCheck
  ...
  if strcmp(subpop, 'all')
    filenamePositive = [rootFolderPositive filesep 'globalUnits_quality.mat'];
    filenameNegative = [rootFolderNegative filesep 'globalUnits_quality.mat'];
  end
else
  filename = [rootFolder filesep 'globalUnits.mat'];
  if strcmp(subpop, 'all')
    filenamePositive = [rootFolderPositive filesep 'globalUnits.mat'];
    filenameNegative = [rootFolderNegative filesep 'globalUnits.mat'];
  end
end
if fullRun

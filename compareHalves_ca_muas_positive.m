...
% of the same recording for area comparisons (within positive subpopulation
% only).
...
for dbCount = dbEntries_caLocal % Loop through db entries
  ...
  dbStruct = dataStruct.seriesData_ca_muas_positive.(fnsData_ca{dbCount});
  if isempty(dbStruct)
    continue
  end
  ...
  % Save data
  dataString = ['dataStruct.seriesData_ca_muas_positive.' fnsData_ca{dbCount} '.popData.phaseCohHalves = phaseCohHalvesDB;'];
  eval(dataString);
  dataStruct.popData_ca_muas_positive.phaseCohHalves = phaseCohHalves;

...
for dbCount = dbEntries_cLocal % Loop through db entries (area 1)
  ...
  [dbStruct, ~, ~, entryName, ~, ~, shankIDs,...
    ...
    MUAsAll, spk, muas, ~, ~, muaMetadata] = get_dbStruct(dataStruct, find(endsWith(fnsData,seriesName1)));
  ...
  for sca = 1:numel(series_ca{dbCount}) % Loop through comparison areas (area 2)
    ...
    if ~overwrite && isfield(dataStruct, 'seriesData_ca_muas')
      dataString = [animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name)];
      if isfield(dataStruct.seriesData_ca_muas, dataString)
        continue
      end
    end
    
    dbStruct2 = dataStruct.seriesData.([animal '_s' num2str(sca_name)]);
    ...
    dbStruct_ca = dataStruct.seriesData_ca.([animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name)]);
    ...
    if ~isempty(fileList)
      ...
      dataString = ['dataStruct.seriesData_ca_muas.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.phaseCoh = phaseCoh;'];
      eval(dataString);
      dataString = ['dataStruct.seriesData_ca_muas.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.units = muas;'];
      eval(dataString);
      dataString = ['dataStruct.seriesData_ca_muas.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.muaMetadata = muaMetadata;'];
      eval(dataString);
      dataString = ['dataStruct.seriesData_ca_muas.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.popData.phaseCoh = phaseCohDB;'];
      eval(dataString);
      delete([entryName '_*.mat']);
    else
      dataString = ['dataStruct.seriesData_ca_muas.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.phaseCoh = [];'];
      eval(dataString);
      dataString = ['dataStruct.seriesData_ca_muas.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.units = [];'];
      eval(dataString);
      dataString = ['dataStruct.seriesData_ca_muas.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.unitMetadata = [];'];
      eval(dataString);
      dataString = ['dataStruct.seriesData_ca_muas.' animal '_s' num2str(series_c{dbCount}) '__' animal '_s' num2str(sca_name) '.popData.phaseCoh = [];'];
      eval(dataString);
    end

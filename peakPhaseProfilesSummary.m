function fH = peakPhaseProfilesSummary(conditions, areas, areas_reverse, AOI, FOI, areaPhaseFOIindividual_reg, areaPhaseFOIindividual_reverse, drawCI)
% fH = peakPhaseProfilesSummary(conditions, areas, areas_reverse, AOI, FOI,
% areaPhaseFOIindividual_reg, areaPhaseFOIindividual_reverse, drawCI).
%
% Function produces figures with sums of peak phase frequency profiles. The
...
for iCond = 1:numel(conditions)
  for iCombos = 1:size(combos,1)
      ...
      if ~isempty(areaPhaseFOIindividual{iCond}{areaInd})
        for f = 1:numel(FOI)
          if sum(~isnan(areaPhaseFOIindividual{iCond}{areaInd}(:,f)))
            ...
            if strcmpi(areaName, 'VBVsCA')
              phaseRange = [-pi pi];
            elseif strcmpi(areaName, 'CAVsVB') || strcmpi(areaName, 'CA1VsS1') || strcmpi(areaName, 'CA1VsRSC')
              phaseRange = [-0.5*pi 0.5*pi];
            else
              phaseRange = [-0.75*pi 0.75*pi];
            end
            phaseMean(iComp,f) = circPeak(iPhaseFOI(~isnan(iPhaseFOI)), edges, phaseRange);
    ...
    figTitle = ['Peaks only: ' conditions{iCond} ' '];
    for iComp = 1:numel(combo)

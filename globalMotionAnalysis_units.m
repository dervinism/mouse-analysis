...
% and phase frequency histograms for area-to-motion comparisons.
%
% The following data files are produced:
% dataDir\area2motionDir\unitsFolder\histosSubfolder\phaseHistos_units.mat or
%   dataDir\area2motionDir\unitsFolder\histosSubfolder\phaseHistos_units_quality.mat or
%   All of these files contain unit phase frequency profiles, phase frequency
%   histograms, and statistical test results.
%
% Unit phase frequency profile figures are saved in dataDir\area2motionDir\unitsFolder\phaseFrequencyProfilesSubfolder
%   or dataDir\area2motionDir\qualityUnitsFolder\phaseFrequencyProfilesSubfolder.
% Individual unit phase frequency histogram figures and summary suplots are
%   saved in dataDir\area2motionDir\unitsFolder\histosSubfolder or dataDir\area2motionDir\qualityUnitsFolder\histosSubfolder.
% Phase frequency maps are saved in dataDir\area2motionDir\unitsFolder\mapsSubfolder or
%   dataDir\area2motionDir\qualityUnitsFolder\mapsSubfolder.
    ...
    for dbCount = 1:numel(fnsData) % Loop through db entries
      ...
      for iAreaPlusAll = area % Loop through the main and pooled areas
        ...
        for iCondPlusAll = condLoop % Loop through the main and pooled conditions
          ...
          freq = dbStruct.popData.motion.popData.freq;
          ...
          for sh = 1:numel(shankIDs)
            ...
            for u = 1:numel(units)
              ...
              coh = shankStruct.motion.unitData{iU}.coh;
              cohConf = shankStruct.motion.unitData{iU}.coh_conf;
              rateadjust_kappa = shankStruct.motion.unitData{iU}.rateadjust_kappa;
              phase = shankStruct.motion.unitData{iU}.phase;
              phaseConfU = shankStruct.motion.unitData{iU}.phase_confU;
              phaseConfL = shankStruct.motion.unitData{iU}.phase_confL;
              ...
              if ~sum(~isnan(freq)) && sum(~isnan(coh))
                freq = shankStruct.motion.unitData{iU}.freq;
              end
              ...
              areaPhaseSpanIndividual{iCondPlusAll}{iAreaPlusAll} = [areaPhaseSpanIndividual{iCondPlusAll}{iAreaPlusAll}; abs(max(phase,[],2)-min(phase,[],2))];
              areaBetaIndividual{iCondPlusAll}{iAreaPlusAll} = [areaBetaIndividual{iCondPlusAll}{iAreaPlusAll}; shankStruct.phaseCoh{iU}.beta];
...
if qualityCheck
  filename = [mainFolder filesep 'globalUnits_area2motion_quality.mat'];
else
  filename = [mainFolder filesep 'globalUnits_area2motion.mat']; %#ok<*UNRCH>
end
if fullRun
...
if drawPhaseProfiles
  figFileName = 'Means_only__%sVsMotion';
  options = struct();
  options.figTitle = 'Unit mean phase comparisons: %sVsMotion';
...
if drawCohProfiles
  figFileName = 'Means_only__%sVsMotion';
  options = struct();
  options.figTitle = 'Unit mean coherence comparisons: %sVsMotion';
  options.figSize = figSize;
  options.mainFolder = mainFolder;
...
if drawBetaCorrelations
  if ~exist([mainFolder filesep betaCorrelationsSubfolder], 'file')
  ...
  for iCond = 1:numel(conditions(1:min([numel(conditions) 2])))
    for iArea = 1:numel(iAreasOI)
      ...
      if ~isempty(beta) && ~isempty(coh)
        ...
        figTitle = ['Unit PSD beta and motion coherence correlations for ' areas{iAreasOI(iArea)} ' ' conditions{iCond} ' '...
        ...
        ax1 = axesProperties({}, 1, 'normal', 'off', 'w', 'Calibri', 12, 4/3, 2, [0.005 0], 'out',...
          'on', 'k', 'Unit PSD exponent \Beta', xlim, [0 0.2 0.4 0.6 0.8 1],...
          'on', 'k', 'Coherence with motion', [-0.05 1.05], [0 0.2 0.4 0.6 0.8 1]);
        title(figTitle);
        figName = ['PSDbetaVsMotionCoherence_in_' areas{iAreasOI(iArea)} 'VsMotion_during_' conditions{iCond} '_at_'...
...
%% Local functions
function [fH, stats, dataScatter, areas, colourGroups] = barPlotUnits(data, areas, condition, yLim, scaleType)
